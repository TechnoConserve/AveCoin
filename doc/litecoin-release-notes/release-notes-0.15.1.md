AveCoin Core version *0.15.1* is now available from:

  <https://download.averyuslaner.com/avecoin/avecoin-0.15.1/>
...
A number of changes to the way AveCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 AVC). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 AVC) in the next release.
