...
- [Development guidelines](#development-guidelines)
    - [General AveCoin Core](#general-avecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play avecoins" on the test network, if you
...
AveCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/avecoin-project/avecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_avecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_avecoin --log_level=test_suite
$ valgrind -v --leak-check=full src/avecoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_avecoin.coverage/index.html`.
```

**Sanitizers**

AveCoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as AveCoin Core, where everyone uses
...
pay attention to for reviewers of AveCoin Core code.

General AveCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: AveCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of AveCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against AveCoin Core so that they can be integrated
...
file descriptors it will cause problems with AveCoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to AveCoin's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:avecoin-project/avecoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `avecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `avecoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of AveCoin Core, the wallet was always
