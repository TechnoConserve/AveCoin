...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/avecoin-project/avecoin>";
    const std::string URL_WEBSITE = "<https://averyuslaner.com/avecoin>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if avecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if avecoin is started while in a temporary directory.\n",
