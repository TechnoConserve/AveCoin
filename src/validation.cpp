...
#if defined(NDEBUG)
# error "AveCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "AveCoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s AVC additional fees, %d delta bytes\n",
