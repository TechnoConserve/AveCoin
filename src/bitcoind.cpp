...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  avecoind [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see avecoind -h for a list of options.\n", argv[i]);
                return false;
            }
        }
...
#endif
            fprintf(stdout, "AveCoin server starting\n");
